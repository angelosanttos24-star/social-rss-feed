...
import { getUserId } from '../api/user-mapping';
...
export const geminiRouter = router({
  ...
  summarizeFeed: protectedProcedure.mutation(async ({ ctx }) => {
    try {
      ...
      console.error('Error in gemini.summarizeFeed:', error);
      throw error;
    }
  }),
  ...
  summarizePost: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      try {
        ...
        if (feedError || feed?.user_id !== userId) {
        ...
        console.error('Error in gemini.summarizePost:', error);
        throw error;
      }
    }),
  ...
  suggestReplies: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      try {
        ...
        console.error('Error in gemini.suggestReplies:', error);
        throw error;
      }
    }),
});
